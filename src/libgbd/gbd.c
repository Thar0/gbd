...
#define OTHERMODE_VAL(state, hi_lo, field) ((state)->othermode_##hi_lo & MDMASK(field))
...
#define DEFINE_WARNING(id, string) GW_##id,
#define DEFINE_ERROR(id, string)   GW_##id,
enum gbi_warning {
...
#define DEFINE_WARNING(id, string) [GW_##id] = string,
#define DEFINE_ERROR(id, string)   [GW_##id] = string,
...
#define DEFINE_WARNING(id, string) [GW_##id] = false,
#define DEFINE_ERROR(id, string)   [GW_##id] = true,
...
#define ARG_CHECK(state, cond, reason, ...) (cond) ? (void)0 : WARNING_ERROR(state, reason, ##__VA_ARGS__)
...
print_light(FILE *print_out, gfx_state_t *state, uint32_t lights_addr, int count)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        fprintf(print_out, "    .col  = { %3d, %3d, %3d },\n", lights.l[i].l.col[0], lights.l[i].l.col[1],
                lights.l[i].l.col[2]);
        fprintf(print_out, "    .pad1 = %d,\n", lights.l[i].l.pad1);
        fprintf(print_out, "    .colc = { %3d, %3d, %3d },\n", lights.l[i].l.colc[0], lights.l[i].l.colc[1],
                lights.l[i].l.colc[2]);
        fprintf(print_out, "    .pad2 = %d,\n", lights.l[i].l.pad2);
        fprintf(print_out, "    .dir  = { %3d, %3d, %3d },\n", lights.l[i].l.dir[0], lights.l[i].l.dir[1],
                lights.l[i].l.dir[2]);
        ...
        case 8: /* CC 2 A */
            {
                static const char *cc1a[] = {
                    "COMBINED", "TEXEL0", "TEXEL1", "PRIMITIVE", "SHADE", "ENVIRONMENT", "1", "NOISE",
                };
        ...
        case 9: /* CC 2 B */
            {
                static const char *cc1b[] = {
                    "COMBINED", "TEXEL0", "TEXEL1", "PRIMITIVE", "SHADE", "ENVIRONMENT", "CENTER", "K4",
                };
        ...
        case 10: /* CC 2 C */
            {
                static const char *cc1c[] = {
                    ...
                    "CENTER",    "COMBINED_ALPHA", "TEXEL0_ALPHA",  "TEXEL1_ALPHA", "PRIMITIVE_ALPHA", "SHADE_ALPHA",
               ...
               int tlut_count)
{
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            // TODO implement transparency in some way for all of these
            switch (fmt_siz) {
...
mtxf_mtxf_mul(MtxF *dst, MtxF *m0, MtxF *m1)
{
    int i;
    for (i = 0; i < 4; i++) {
        dst->mf[0][i] = m0->mf[0][i] * m1->mf[0][0] + m0->mf[1][i] * m1->mf[0][1] + m0->mf[2][i] * m1->mf[0][2] +
                        m0->mf[3][i] * m1->mf[0][3];
        dst->mf[1][i] = m0->mf[0][i] * m1->mf[1][0] + m0->mf[1][i] * m1->mf[1][1] + m0->mf[2][i] * m1->mf[1][2] +
                        m0->mf[3][i] * m1->mf[1][3];
        dst->mf[2][i] = m0->mf[0][i] * m1->mf[2][0] + m0->mf[1][i] * m1->mf[2][1] + m0->mf[2][i] * m1->mf[2][2] +
                        m0->mf[3][i] * m1->mf[2][3];
        dst->mf[3][i] = m0->mf[3][i] * m1->mf[3][3] + m0->mf[2][i] * m1->mf[3][2] + m0->mf[1][i] * m1->mf[3][1] +
                        m0->mf[0][i] * m1->mf[3][0];
    }
}
...
print_vtx(gfx_state_t *state, uint32_t vtx_addr, int v0, int num)
{
    ...
    for (int i = 0; i < num; i++) {
        ...
        gfxd_printf("        { { { %6d, %6d, %6d }, %d, { %6d, %6d }, { %4d, %4d, %4d, %4d } } }\n", vtx.v.ob[0],
                    vtx.v.ob[1], vtx.v.ob[2], vtx.v.flag, vtx.v.tc[0], vtx.v.tc[1], vtx.v.cn[0], vtx.v.cn[1],
                    vtx.v.cn[2], vtx.v.cn[3]);
...
chk_render_primitive(gfx_state_t *state, enum prim_type prim_type, int tile)
{
    ...
    switch (cycle_type) {
        case G_CYC_1CYCLE:
            ...
            ARG_CHECK(state,
                      state->cc.a0 == state->cc.a1 && state->cc.b0 == state->cc.b1 && state->cc.c0 == state->cc.c1 &&
                          state->cc.d0 == state->cc.d1 && state->cc.Aa0 == state->cc.Aa1 &&
                          state->cc.Ab0 == state->cc.Ab1 && state->cc.Ac0 == state->cc.Ac1 &&
...
chk_DPLoadBlock(gfx_state_t *state)
{
    ...
    ARG_CHECK(state,
              timg_phys_low == 0 || timg_phys_low >= 8 || state->last_timg.siz == G_IM_SIZ_4b ||
                  (line_texels * G_SIZ_BITS(state->last_timg.siz) < 58 * 8),
              GW_BAD_TIMG_ALIGNMENT);
...
chk_DPLoadTile(gfx_state_t *state)
{
    ...
    ARG_CHECK(state,
              timg_phys_low == 0 || timg_phys_low >= 8 || state->last_timg.siz == G_IM_SIZ_4b ||
                  (line_texels * G_SIZ_BITS(state->last_timg.siz) / 8 < 58),
              GW_BAD_TIMG_ALIGNMENT);
...
chk_DPLoadTLUTCmd(gfx_state_t *state)
{
    ...
    ARG_CHECK(state,
              timg_fmtsiz == FMT_SIZ(G_IM_FMT_RGBA, G_IM_SIZ_16b) || timg_fmtsiz == FMT_SIZ(G_IM_FMT_IA, G_IM_SIZ_16b),
              GW_TLUT_BAD_FMT);
...
ltb_width_valid(int width, int siz)
{
    int valid_16b_widths[] = {
        4,   8,   12,  16,  20,  24,  28,  32,  36,  40,  48,  64,  72,  76,  100, 108,
        128, 144, 152, 164, 200, 216, 228, 256, 304, 328, 432, 456, 512, 684, 820, 912,
    };
...
static int
chk_LTB(gfx_state_t *state, uint32_t timg, int fmt, int siz, int width, int height, int pal, unsigned cms, unsigned cmt,
        ...
        case gfxd_Color:
            if (m_id == gfxd_DPSetFillColor && state->cimg_set) {
                const char *color_fmt =
                    (state->options->hex_color) ? "0x%02X, 0x%02X, 0x%02X, 0x%02X" : "%d, %d, %d, %d";
                ...
                switch (FMT_SIZ(state->last_cimg.fmt, state->last_cimg.siz)) {
                    case FMT_SIZ(G_IM_FMT_RGBA, G_IM_SIZ_16b):
                        if (state->zimg_set && state->last_cimg.addr == state->last_zimg.addr)
                            ...
                            gfxd_printf("(GPACK_ZDZ(0x%X, 0x%X) << 16) | GPACK_ZDZ(0x%X, 0x%X)",
                                        ZDZ_UNPACK_Z(color >> 16), ZDZ_UNPACK_DZ(color >> 16), ZDZ_UNPACK_Z(color >> 0),
                                        ZDZ_UNPACK_DZ(color >> 0));
                        else {
                            // otherwise write as rgba16
                            gfxd_puts("(GPACK_RGBA5551(");
                            gfxd_printf(color_fmt, RGBA16_EXPAND(RGBA16_R(color >> 16)),
                                        RGBA16_EXPAND(RGBA16_G(color >> 16)), RGBA16_EXPAND(RGBA16_B(color >> 16)),
                                        255 * RGBA16_A(color >> 16));
                            gfxd_puts(") << 16) | GPACK_RGBA5551(");
                            gfxd_printf(color_fmt, RGBA16_EXPAND(RGBA16_R(color >> 0)),
                                        RGBA16_EXPAND(RGBA16_G(color >> 0)), RGBA16_EXPAND(RGBA16_B(color >> 0)),
                                        255 * RGBA16_A(color >> 0));
                            gfxd_puts(")");
                        }
                        break;
                    ...
                    case FMT_SIZ(G_IM_FMT_CI, G_IM_SIZ_8b):
                        if (state->options->hex_color)
                            ...
                            gfxd_printf(color_fmt, (color >> 24) & 0xFF, (color >> 16) & 0xFF, (color >> 8) & 0xFF,
                                        (color >> 0) & 0xFF);
                            gfxd_puts(")");
                        }
                        break;
        ...
        case USE_START_ADDR_AT_POINTER:
            {
                ...
                if (!state.rdram->read_at(&auto_start_addr, start_location->start_location_ptr & ~KSEG_MASK,
                                          sizeof(uint32_t))) {
